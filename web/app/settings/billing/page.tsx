...
export default function BillingSettingsPage() {
  ...
  useEffect(() => {
    setSub(store.getSubscription());
  }, []);

  function applyPlan(plan: Plan) {
    const now = new Date();
    const renewsAt = new Date();
    if (plan === "operator-monthly") renewsAt.setMonth(renewsAt.getMonth() + 1);
    else if (plan === "operator-annual")
    ...
    const next: Subscription = {
      ...
      renewsAtIso:
        plan === "free" || plan === "founder" ? null : renewsAt.toISOString(),
      founderSeatNumber:
        plan === "founder"
          ? FOUNDER_PASS_SEED_CLAIMED + 1
          : sub.founderSeatNumber,
    };
    setSub(next);
      ...
      {!isFounder ? (
        ...
        >
          ...
          <p className="mt-4 text-center font-mono text-[10px] uppercase tracking-[0.22em] text-ink-300/55">
            Demo flow &mdash; real impl routes to Stripe Checkout (web) or Apple/Google IAP (native).
          </p>
        </Section>
      ) : null}
        ...
        >
          {!showCancelConfirm ? (
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                Switch to{" "}
                {sub.plan === "operator-monthly" ? "annual" : "monthly"}
              </Button>
              <button
                type="button"
